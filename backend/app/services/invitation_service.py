...
def accept(*, user_id: uuid.UUID | str, raw_token: str) -> SwitchResult:
    """Consume an invitation, create the membership, and auto-switch into the household.

    No-tenant mode: the invitee isn't a member yet, so RLS can't guard this. The
    **email-binding check** (accepting user's email == invited email) is the security gate.
    Idempotent: an already-existing membership is treated as success.
    """
    with session_scope(bypass_tenant=True) as session:
        invitation = _validate_pending(
            invitations_repo.get_by_token_hash(session, _hash(raw_token))
