 ...
 */
export async function waitForActivationToken(api, email, timeoutMs = 15_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const search = await api.get(`${MAILPIT_BASE}/api/v1/search?query=${encodeURIComponent(`to:${email}`)}`);
        if (search.ok()) {
            ...
            if (id) {
                ...
                const match = `${body.Text ?? ''}${body.HTML ?? ''}`.match(/\/activate\/([A-Za-z0-9_-]+)/);
                if (match)
                    return match[1];
            }
        }
