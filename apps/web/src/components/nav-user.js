      ...
      <SidebarMenuItem>
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-(--radix-dropdown-menu-trigger-width) min-w-56 rounded-lg" side={isMobile ? 'bottom' : 'right'} align="end" sideOffset={4}>
            ...
            {memberships.length > 0 ? (<DropdownMenuRadioGroup value={activeHousehold?.household_id}>
                {memberships.map((m) => (<DropdownMenuRadioItem key={m.household_id} value={m.household_id}>
                    <span className="truncate">{m.household_name}</span>
                  </DropdownMenuRadioItem>))}
              </DropdownMenuRadioGroup>) : (<DropdownMenuItem disabled className="text-xs">
                —
              </DropdownMenuItem>)}
