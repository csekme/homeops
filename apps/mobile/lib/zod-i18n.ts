/**
 * Global Zod error map → i18n (phase0-mobile §9). Identical strategy to the web app:
 * the shared `@homeops/validation` schemas omit human-readable messages, so this maps
 * Zod issue *codes* to localized strings from the `validation` namespace. Imported for
 * side-effect from the root layout, after i18n init.
 */
import { z } from 'zod';
