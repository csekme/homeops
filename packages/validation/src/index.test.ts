...
describe("householdSchema", () => {
  it("validates name + currency", () => {
    expect(
      householdSchema.safeParse({ name: "Home", default_currency: "HUF" }).success,
    ).toBe(true);
    expect(
      householdSchema.safeParse({ name: "Home", default_currency: "huf" }).success,
    ).toBe(false);
...
describe("inviteSchema", () => {
  ...
  it("rejects an unknown role", () => {
    expect(
      inviteSchema.safeParse({ email: "a@b.com", role: "SUPERUSER" }).success,
    ).toBe(false);
  });
});
...
describe("obligationSchema", () => {
  it("accepts a minimal one-off obligation", () => {
    ...
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.lead_time_days).toBe(0);
  });
