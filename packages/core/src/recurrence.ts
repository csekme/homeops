...
export function nextOccurrence(rrule: string, after: Date): Date | null {
  ...
  try {
    ...
    if (hasDtstart(parsed)) {
      rule = parsed as RRule;
    } else {
      // No DTSTART in the rule: re-parse with `after` as the anchor so the
      // schedule is evaluated relative to the query point.
      rule = new RRule({
        ...(parsed as RRule).origOptions,
        dtstart: after,
      });
    }
  } catch (err) {
    throw new RecurrenceError(`Failed to parse RRULE "${rrule}": ${(err as Error).message}`);
  }
...
function hasDtstart(parsed: RRule | { origOptions?: { dtstart?: Date | null } }): boolean {
  const opts = (parsed as RRule).origOptions;
